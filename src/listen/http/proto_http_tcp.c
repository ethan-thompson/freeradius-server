...
have_packet:
	...
	invalid:
		{
			static char const bad_request[] = "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";

			PERROR("Invalid HTTP message");
			if (write(thread->sockfd, bad_request, sizeof(bad_request) - 1) < 0) {
				WARN("proto_http_tcp - failed sending 400 response: %s", fr_syserror(errno));
			}
		}
		return -1;
	}
			 ...
			 uint8_t *buffer, size_t buffer_len, size_t written)
{
	...
	if ((size_t)(data_size + written) == buffer_len) {
		uint8_t const *blank_line = memmem(buffer, buffer_len, "\r\n\r\n", 4);
		size_t header_len = blank_line ? (size_t)(blank_line - buffer) : buffer_len;

		if (memmem(buffer, header_len, "Connection: close", 17)) {
			DEBUG2("proto_http_tcp - closing connection after response (Connection: close) %s", thread->name);
			errno = ECONNRESET;
			return -1;
		}
	}
