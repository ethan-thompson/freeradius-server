...
static fr_dict_attr_t const *attr_http_request_version;
static fr_dict_attr_t const *attr_http_header;
static fr_dict_attr_t const *attr_http_header_name;
static fr_dict_attr_t const *attr_http_header_value;
...
static int mod_decode(UNUSED void const *instance, request_t *request, uint8_t *const data, size_t data_len)
{
	...
	fr_dbuff_init(&dbuff, request->packet->data, request->packet->data_len);
	 ...
	 *	is known yet.
	 */
	if (fr_http_decode(request->request_ctx, &request->request_pairs, &dbuff) < 0) {
...
static ssize_t mod_encode(UNUSED void const *instance, request_t *request, uint8_t *buffer, size_t buffer_len)
{
	...
	/*
	 *	RFC 9112 §9.3: HTTP/1.0 defaults to closing the connection after
	 *	the response unless the request asked to keep it open; HTTP/1.1
	 *	defaults to keeping it open unless the request asked to close it.
	 *	proto_http_tcp's mod_write is responsible for actually closing the
	 *	socket once it sees the "Connection: close" header this produces.
	 */
	{
		bool http_1_0 = false, explicit_close = false, explicit_keepalive = false;
		fr_pair_t *request_vp = fr_pair_find_by_da(&request->request_pairs, NULL, attr_http_request);

		if (request_vp) {
			fr_pair_t *version_vp = fr_pair_find_by_da(&request_vp->vp_group, NULL, attr_http_request_version);
			if (version_vp && (version_vp->vp_uint8 == 0)) http_1_0 = true;
		}

		{
			...
			for (header_vp = fr_pair_dcursor_by_da_init(&cursor, &request->request_pairs, attr_http_header);
			     header_vp != NULL;
				...
				if (strcasecmp(name_vp->vp_strvalue, "Connection") != 0) continue;

				if (strcasecmp(value_vp->vp_strvalue, "close") == 0) explicit_close = true;
				if (strcasecmp(value_vp->vp_strvalue, "keep-alive") == 0) explicit_keepalive = true;
			}
		}

		encode_ctx.close = explicit_close || (http_1_0 && !explicit_keepalive);
	}
