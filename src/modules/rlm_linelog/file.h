...
struct rlm_linelog_file_s {
	...
	fr_dbuff_t			dbuff;			//!< Talloced, dynamically resized dbuff containing all linelog content to write out.
	...
	size_t				total_data_len;		//!< Total data length in all entries.
...
void batching_cleanup(rlm_linelog_file_t *file);

unlang_action_t batching_mod_resume(UNUSED unlang_result_t *p_result, UNUSED module_ctx_t const *mctx, UNUSED request_t *request);

xlat_action_t batching_xlat_resume(UNUSED TALLOC_CTX *ctx, UNUSED fr_dcursor_t *out, xlat_ctx_t const *xctx, request_t *request, UNUSED fr_value_box_list_t *in);
